...
new #[Title('Gestión de Razas')] class extends Component {
    ...
    public bool $confirmingRazaDeletion = false;
    public $razaIdDeletion = null;

    public ?Breed $razaToEdit = null;
    ...
    public function razas()
    {
        return Breed::with('species')
            ->when($this->search, function ($query) {
                $query->where('name', 'like', '%' . $this->search . '%')->orWhereHas('species', function ($q) {
    ...
    {
        return \App\Models\BreedRequest::with(['species', 'veterinary', 'user'])
    ...
    public function approveRequest(int $id): void
    {
        ...
        $request = BreedRequest::findOrFail($id);

        $speciesId = $request->species_id;

        if (!$speciesId && $request->custom_species) {
            $species = Species::firstOrCreate([
                'name' => $request->custom_species,
            ]);
        ...
        Pet::where('veterinary_id', $request->veterinary_id)
            ->where('breed_custom', $request->name)
        ...
        $this->dispatch('notify', message: 'Raza aprobada y mascotas actualizadas.', type: 'success');
    }
    ...
    public function rejectRequest(int $id): void
    {
        ...
        $request = \App\Models\BreedRequest::findOrFail($id);
    ...
    public function toggleCreateForm(): void
    {
        ...
        $this->razaToEdit = null;
        $this->showCreateForm = true;
        $this->showModal = true;
    }

    public function editRaza(int $id): void
    {
        Gate::authorize('manage-veterinarias');

        $this->razaToEdit = Breed::findOrFail($id);
        $this->showEditForm = true;
        $this->showCreateForm = false;
        $this->showModal = true;
    }

    public function confirmRazaDeletion(int $id): void
    {
        Gate::authorize('manage-veterinarias');

        $this->razaIdDeletion = $id;
        $this->confirmingRazaDeletion = true;
    }

    public function deleteRaza(): void
    {
        Gate::authorize('manage-veterinarias');

        Breed::findOrFail($this->razaIdDeletion)->delete();

        $this->confirmingRazaDeletion = false;
        $this->razaIdDeletion = null;
        $this->dispatch('notify', message: 'Raza eliminada correctamente', type: 'success');
    }
    ...
    {
        $this->reset(['showCreateForm', 'showEditForm', 'showModal', 'razaToEdit']);
    }
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="md:text-2xl text-xl font-semibold text-white">Gestión de Razas</h1>
            <p class="text-gray-400 mt-1">Administración de razas por especie</p>
        </div>
    ...
    <x-modal show="showModal" wire:click="handleFormCancelled" maxWidth="max-w-xl">
        @if ($showEditForm && $razaToEdit)
            <livewire:forms.breed :raza="$razaToEdit" :key="'edit-' . $razaToEdit->id" />
        @elseif ($showCreateForm)
            <livewire:forms.breed />
        @endif
    </x-modal>

    <!-- Tabs -->
    <div class="flex border-b border-gray-700 mb-6 font-outfit">
        <button wire:click="setTab('all')"
            class="px-6 py-3 text-sm font-medium transition-all relative {{ $activeTab === 'all' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-400 hover:text-white' }}">
            Todas las Razas
        </button>
        <button wire:click="setTab('pending')"
            class="px-6 py-3 text-sm font-medium transition-all relative {{ $activeTab === 'pending' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-400 hover:text-white' }}">
        ...
        <div class="bg-gray-800 rounded-lg overflow-hidden border border-gray-700">
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-950 text-gray-300">
                    <tr
                        class="[&>th]:px-3 [&>th]:md:px-6 [&>th]:py-3 [&>th]:md:py-4 [&>th]:text-left font-outfit uppercase text-xs tracking-wider">
                        <th>Especie</th>
                        <th>Raza</th>
                        <th>Acciones</th>
                    </tr>
                ...
                <tbody class="divide-y divide-gray-700">
                    @foreach ($this->razas as $raza)
                        <tr wire:key="raza-{{ $raza->id }}"
                            class="hover:bg-gray-750 transition-colors [&>td]:px-2 [&>td]:md:px-6 [&>td]:py-2 [&>td]:md:py-4">
                            <td>
                                <p class="text-white font-medium">{{ $raza->species?->name }}</p>
                            </td>
                            <td>
                                <p class="text-white font-medium">{{ $raza->name }}</p>
                            </td>

                            <td>
                                <div class="flex md:gap-4 gap-3 items-center">
                                    @can('manage-veterinarias')
                                        <button wire:click="editRaza({{ $raza->id }})"
                                        ...
                                        <button wire:click="confirmRazaDeletion({{ $raza->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p>No hay solicitudes de razas pendientes.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
